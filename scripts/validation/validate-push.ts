...
async function main(): Promise<void> {
  ...
  if (validatesCode) {
    await runSteps(LIVE_PUSH_VALIDATION_POLICY.codeSteps);
  }

  if (scopes.has("product")) {
    if (!validatesCode) {
      await runScript(LIVE_PUSH_VALIDATION_POLICY.productFormatStep);
    }
    await runSteps(LIVE_PUSH_VALIDATION_POLICY.productSteps);
  }
